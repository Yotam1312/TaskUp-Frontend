...
const translations = {
  ...
  he: {
    ...
    close: 'סגור',
    title: 'תקנון, תנאי שימוש ומדיניות פרטיות',
    ...
    <>
      <TouchableOpacity
        ...
        style={{
          ...
          alignItems: 'center',
          gap: 12,
          paddingVertical: 8, // שטח לחיצה גדול יותר
        }}
      >
        <Checkbox
          value={isAccepted}
      ...
      <Modal visible={isModalVisible} transparent animationType="fade" onRequestClose={closeModal}>
        <Pressable style={[StyleSheet.absoluteFillObject, { backgroundColor: 'rgba(0,0,0,0.5)' }]} onPress={closeModal} />
        ...
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 }}>
          <View style={{ 
            ...
            <Text style={{ color: headingText, fontSize: 20, fontWeight: '800', marginBottom: 8, textAlign: 'center' }}>
              {t.title}
            </Text>

            <Text style={{ color: isDark ? '#94a3b8' : '#64748b', fontSize: 13, fontWeight: '600', marginBottom: 16, textAlign: 'center' }}>
              {t.updatedAt}
            </Text>

            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.contentContainer} style={{ flexGrow: 1 }}>
              {t.sections.map((section, sectionIndex) => (
                <View key={`${section.heading}-${sectionIndex}`} style={{ marginBottom: 16 }}>
                  <Text style={{ color: headingText, fontSize: 16, fontWeight: '800', marginBottom: 10, textAlign: currentLanguage === 'he' ? 'right' : 'left' }}>
                    {section.heading}
                  </Text>

                  {section.items.map((item, itemIndex) => (
                    <View key={`${section.heading}-${itemIndex}`} style={{ flexDirection: currentLanguage === 'he' ? 'row-reverse' : 'row', alignItems: 'flex-start', marginBottom: 12 }}>
                      <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: '#4f46e5', marginTop: 7, marginHorizontal: 10 }} />
                      <Text style={{ color: bodyText, fontSize: 14, lineHeight: 22, flex: 1, textAlign: currentLanguage === 'he' ? 'right' : 'left' }}>
                        {item.text}
                      </Text>
                    </View>
                  ))}
            ...
            <TouchableOpacity onPress={closeModal} activeOpacity={0.85} style={[styles.closeButton, { backgroundColor: closeButtonBg }]}>
